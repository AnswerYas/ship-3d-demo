<template>
    ...
    <div id="canvas" ref="canvas" v-show="sceneType === 'earth'"></div>
    <ship @onBack="onBack" v-show="sceneType === 'ship'" />
</template>
...
const getAssetsFile = (path) => `./earth-3d/${path}`;

const sceneType = ref<string>("earth");
...
const initLight = (): void => {
    const ambientLight: THREE.AmbientLight = new THREE.AmbientLight(
        new THREE.Color("rgb(222, 237, 255)")
    );
...
const createEarth = () => {
    ...
    // 大气层
    const cloudGeo: THREE.SphereGeometry = new THREE.SphereGeometry(
        5.1,
        40,
        40
    );
    const cloudTexture: THREE.Texture = textureLoader.load(
        getAssetsFile("earth_cloud.png")
    );
...
const createSatellite = (): void => {
    mTLLoader.load(getAssetsFile("/Satellite.mtl"), (material: any) => {
        // 预加载材质所需的所有纹理、贴图
        material.preload();

        objLoader
            .setMaterials(material)
            .load(getAssetsFile("/Satellite.obj"), (obj: any) => {
...
const createEarthPoint = (
    localton: THREE.Vector3,
    color: string
): THREE.Group => {
    // 新建一个标点组合
    ...
    const lightGeo: THREE.CylinderGeometry = new THREE.CylinderGeometry(
        0,
        0.05,
        0.5,
        32
    );
    const lightTexture: THREE.Texture = textureLoader.load(
        getAssetsFile("lightray.png")
    );
...
const drawPointOnEarth = (): void => {
    ...
    const ship = textureLoader.load(getAssetsFile("ship.png"));
    const shipSpriteMaterial = new THREE.SpriteMaterial({
        map: ship,
        sizeAttenuation: false,
    });
    const shipSprite = new THREE.Sprite(shipSpriteMaterial);
    shipSprite.scale.set(0.05, 0.05, 0.05);
    const xyz = lglnToxyz(160, 22, 5.1);
    shipSprite.position.set(xyz.x, xyz.y, xyz.z);
    shipSprite.name = "ship";
    earthGroup.add(localtionGroup, flyLineGroup, shipSprite);
};
...
const createFlyLine = (v0: THREE.Vector3, v3: THREE.Vector3): THREE.Line => {
    ...
    const centerPoint: THREE.Vector3 = v0
        .clone()
        .add(v3.clone())
        .divideScalar(2);
    // 从圆心到中间点形成无穷远的射线
...
const render = (): void => {
    ...
    if (flyLineArr.length) {
        flyLineArr.forEach((flyLine) => {
            ...
            let flyLinePoints = p.splice(
                (flyLineGeo as any)._index,
                (flyLineGeo as any)._index + num
            );
